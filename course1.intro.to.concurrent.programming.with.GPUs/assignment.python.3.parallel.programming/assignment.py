...
def execute_ticketing_system_participation(ticket_number, part_id, shared_variable: deque):
    ...
    while not shared_variable or shared_variable[0] != ticket_number:
        time.sleep(1) 
    shared_variable.popleft()
...
class Assignment(Core):
    ...
    def run(self):
        ...
        if self.test_username_equality(self.USERNAME):
            sleeping_time = 0
            ## current_thread_count = active_count()
            for index in range(self.num_threads):
                ...
                ##lock = Lock()
                ##lock.acquire()
                ##self.ticket_locks.append(lock)
                
                thread = Thread(group=None, target=execute_ticketing_system_participation, args=(self.ticket_id + index, self.part_id, self.deque))
                thread.start()
            ...
            ##while active_count() < current_thread_count + self.num_threads:
            ##    time.sleep(1)    
            #logging.info("All threads were started.")    

            ##for idx in range(self.num_threads):
            ##    self.ticket_locks[idx].release()
    ...
    def manage_ticketing_system(self):
        ...
        for _ in range(self.num_threads):
            self.deque.append(self.ticket_id)
            self.ticket_id += 1

        for idx_2 in range(self.num_threads):
            self.active_threads[idx_2].join()
        return 0
