...
void executeTicketingSystemParticipation(int ticketNumber) {
	...
	while (currentTicketNumber.load() != ticketNumber)
	{
...
int runSimulation() {
	...
	if(USERNAME == currentUser && USERNAME == userFromFile){
		...
		int startTicketNumber = currentTicketNumber;
		...
		for(int threadIndex = 0; threadIndex < currentNumThreads; ++threadIndex){
			...
			///threads[threadIndex] =
			std::thread(executeTicketingSystemParticipation, startTicketNumber + threadIndex).detach();
			//threads[threadIndex].detach();
		}
		//gloablThreads = threads;
...
//int manageTicketingSystem(std::thread threads[])
int manageTicketingSystem()
{
	...
	// 	threads[i].join();
	// }
	while (currentTicketNumber.load() < currentNumThreads) {
		std::cout << "xxx 5\n";
		std::this_thread::sleep_for(std::chrono::seconds(1));
	}
