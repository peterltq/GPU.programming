...
__global__ void convert(uchar *d_r, uchar *d_g, uchar *d_b, uchar *d_gray)
{
    ...
    uchar avg = (d_r[threadId] + d_g[threadId] + d_b[threadId]) / 3;
    d_gray[threadId] = avg;
}
...
__host__ void executeKernel(uchar *d_r, uchar *d_g, uchar *d_b, uchar *d_gray, int rows, int columns, int threadsPerBlock)
{
    ...
    int blockZSize = 3; // Could consider making the block/grid and memory layout 3d mapped but for now just breaking up computation
    int gridCols = min(columns/(threadsPerBlock*blockZSize),1);
