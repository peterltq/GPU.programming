...
__global__ void add(float *d_a, float *d_b, float *d_c, int numElements)
{
    int i = blockDim.x * blockIdx.x + threadIdx.x;
...
__host__ std::tuple<float *, float *> allocateRandomHostMemory(int numElements)
{
    ...
    // 1. Set device flag for mapped memory
...
__host__ void copyFromHostToDevice(float *h_a, float *h_b, float *d_a, float *d_b, int numElements)
{
    ...
    cudaError_t err = cudaHostGetDevicePointer((float **)&d_a, (float *)h_a, 0);
 
    if (err != cudaSuccess)
    {
        fprintf(stderr, "cudaHostGetDevicePointer: Failed to copy vector A from host to device (error code %s)!\n", cudaGetErrorString(err));
        exit(EXIT_FAILURE);
    }

    //err = cudaHostGetDevicePointer((float **)&d_b, (float *)h_b, size);
    err = cudaHostGetDevicePointer((float **)&d_b, (float *)h_b, 0);
    if (err != cudaSuccess)
    {
