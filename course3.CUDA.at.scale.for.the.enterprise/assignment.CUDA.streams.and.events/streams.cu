...
__host__ float * runStreamsFullAsync(float *host_mem, int num_elements)
{
    ...
    int a2Seed;
    std::cout << "Enter a seed for A2: ";
    std::cin >> a2Seed;
    a2Seed = a2Seed % 256;
    srand(a2Seed);
    float y = static_cast <float> (rand()) / (static_cast <float> (RAND_MAX/random_max));

    kernelA2<<<blocksPerGrid,threadsPerBlock,0, stream4>>>(dev_mem, num_elements, y);
...
__host__ float * runStreamsBlockingKernel2StreamsNaive(float *host_mem, int num_elements)
{
    ...
    kernelA2<<<blocksPerGrid,threadsPerBlock,0, stream3>>>(dev_mem, num_elements, y);
