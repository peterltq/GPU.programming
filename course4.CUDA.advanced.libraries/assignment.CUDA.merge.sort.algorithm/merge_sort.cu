...
__host__ void printDeviceMemory(long *dev_mem, long start, long len)
{
...
__host__ long* mergesort(long* data, long numElements, dim3 threadsPerBlock, dim3 blocksPerGrid) {
    ...
    for (long width = 2; width < (numElements << 1); width <<= 1) {
        ...
        checkCudaErrors(cudaMemcpy(data, B, numElements * sizeof(long), cudaMemcpyDeviceToHost));
        // printDeviceMemory(data, 0, numElements);
