...
__global__ void complexProcess(Complex *a, Complex *b, Complex *c, int size, int scalar)
{
    ...
    // TODO process complexScaleMult on values in a and b at index threadID and the passed scalar, place the result in c[threadId]
    if (idx > size) {
        return;
    }

    c[idx] = complexScaleMult(a[idx], b[idx], scalar);
}
...
__host__ cufftComplex *generateCuFFTComplexPointerFromHostComplex(int mem_size, Complex *hostComplex)
{
    ...
    checkCudaErrors(cudaMemcpy(d_signal, hostComplex, mem_size, cudaMemcpyHostToDevice));
